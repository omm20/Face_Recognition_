...
#third image for student

        img2 = Image.open(r"photos/logo-m3.png")
...
#background image 
        img3 = Image.open(r"photos/up_img1.jpg")
        ...
        title_lbl = Label(bg_img,text="Student Attendance System",font=("times new roman",35,"bold"),bg="white",fg="black")
        ...
        # current course
        current_course_frame = LabelFrame(Left_frame,bd=2,bg="White",relief=RIDGE,text="Current courese info",font=("times new roman",12,"bold"))
        ...
        dep_combo["values"]=("select deapartment","COmputer","it","civil","mechanical")
        dep_combo.current(0)
...
#Student Id
       ...
       # gender_entry.grid(row=2,column=1,padx=10,pady=5,sticky=W)
        ...
        #Email
        email_lable = Label(class_Student_frame,text="E-mail:",font=("times new roman",13,"bold"),bg="white")
...
#new button frame 
        ...
        img_right = Image.open(r"photos/logo-m2.png")
    ...
    def add_data(self): 
        if self.var_dep.get()=="Select Department" or self.var_std_name.get()=="" or self.var_std_id.get()=="":
            ...
            try:
                conn = mysql.connector.connect(host="localhost",username="root",password="omm@123",database="face_recognizer")
        ...
        if (self.var_dep.get() == "Select Department" or 
            ...
            try:
                ...
                if Update:
                    ...
                    self.fetch_data()  # Ensure this function works properly
                    conn.close()
                else:
                    messagebox.showinfo("Info", "Update cancelled", parent=self.root)
            except Exception as es:
                messagebox.showerror("Error", f"Due to: {str(es)}", parent=self.root)
     #delete function
    def delete_data(self):
         if self.var_std_id.get()=="":
              messagebox.showerror("Eoor","styudent id must be req",parent=self.root)
         else:
              try:
                   delete=messagebox.askyesno("Student Delete page","Do uh wnt to del thuis student",parent=self.root)
                   if delete>0:
                        conn = mysql.connector.connect(host="localhost", username="root", password="omm@123", database="face_recognizer")
                        my_cursor = conn.cursor()
                   ...
                   self.fetch_data()
                   conn.close()
                   messagebox.showinfo("Delete","Success Deleteed",parent=self.root)
              except Exception as es:
                messagebox.showerror("Error", f"Due to: {str(es)}", parent=self.root)
    ...
    def generate_dataset(self):
        if (self.var_dep.get() == "Select Department" or 
            ...
            try:
                ...
                #=====load oredefined data face frontal open cv
